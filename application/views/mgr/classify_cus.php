                    ...
                    <div class="panel-body">
                        <div class="m-t-10">
                            <table class="table horizontal_table table-striped">
                                ...
                                <tbody>
                                    <form action="<?= base_url() ?>mgr/funiture/classify/add" method="POST">
                                        <td>
                                ...
                                <tbody>
                                    <? for ($i = 0; $i < count($list); $i++) {
                                        ...
                                        <tr>
                                            <td><?= $i + 1 ?></td>
                                            <td><?= $item['classify'] ?></td>
                                            <td><button onclick="location.href='<?= base_url() . "mgr/funiture/category/index/" . $item['id'] ?>'" class="btn btn-success btn">子分類列表</button></td>

                                            <td><?= $item['create_date'] ?></td>
                                            <td style="width:10%;">
                                                <button onclick="location.href='<?= base_url() . "mgr/funiture/classify/edit/" . $item['id'] ?>'" class="btn btn-primary btn-xs"><span class="fa fa-fw ti-pencil"></span></button>
                                                <button onclick="location.href='<?= base_url() . "mgr/funiture/classify/delete/" . $item['id'] ?>'" class="btn btn-danger btn-xs"><span class="fa fa-fw ti-trash"></span></button>
                                            </td>
                                        </tr>
                                    <? } ?>
                                </tbody>

...
class Notification extends Base_Controller {
	private $th_title = ["使用者", "標題","內文","發送日期"]; //, "置頂"
	private $th_width = ["", "", "", ""];
	private $order_column = ["user_id", "", "",""]; //, "is_head"
	private $can_order_fields = [];

	private $user_id  =['all_collection','ready_to_wear','customize','shop'];
	...
	public function index(){
		$this->data['title'] = '通知中心';
		$this->data['excel_export'] = false;
		$this->data['export_url'] = "mgr/notification/export";
		$this->data['action'] = base_url()."mgr/notification/";

		$this->data['th_title'] = $this->th_title;
		$this->data['th_width'] = $this->th_width;
		$this->data['can_order_fields'] = $this->can_order_fields;
		$this->data['tool_btns'] = [
			['新增通知', base_url()."mgr/notification/add", "btn-primary"]
		];
	...
	public function data(){
		...
		$total = $this->db->from($this->notification." N")
		...
		$list = $this->db->select("N.*,U.*")
						 ->from($this->notification." N")
						//  ->join($this->faq_classify_table." S", "S.id = P.classify", "left")
		...
		foreach ($list as $item) {
			...
			$html .= $this->load->view("mgr/items/notification_item", array(
				"item"  =>	$item,
				"total" =>	$total_num
			), TRUE);
		}
	...
	public function add(){
		...
		$param = [								
								...
								// ["職稱", "title", "text", $oridata['title']],	["性別", "gender", "select",$oridata['gender'],["id",'gender']],			
								// ["生日", "birthday", "day",$oridata['birthday']],
								["對象", "user_id", "select","",["id",'username']],
								// ["狀態", "user_id", "select",$oridata['username'],["id",'username']],
								// ["對象", "user_id", "text",""],
								// ["留言", "remark", "plain", $oridata['remark']],
		...
		if ($_POST) {
			...
			if($data['user_id']==0){
				$target_user = $this->db->get_where('user',["is_delete"=>0])->result_array();
				...
				foreach($target_user as $user){
					...
					// print_r($data_one);
					$res = $this->db->insert("notification", $data_one);
				}
				// exit;
			}else{
			...
			if ($res) {
				...
				$this->js_output_and_redirect("編輯成功", base_url()."mgr/notification");
			}else{
				$this->js_output_and_back("發生錯誤");
			}
		}else{
			//僅讀取add頁面
			$this->data['title'] = '新增通知 ';
			$this->data['parent'] = '通知中心';
			$this->data['parent_link'] = base_url()."mgr/user";
			$this->data['action'] = base_url()."mgr/notification/add";
			$this->data['submit_txt'] = "確認新增";
			
			// $this->data['select']['user_id'] = 
			// array(
			// 	0=>array(
			// 		'id'=>'all_collection',
			// 		'user_id'=> '正常'),
		...
		$this->data['select']['user_id'] = $user_data;
			// $this->data['select']['gender'] = 
			// $this->data['select']['user_id'] = array(['id' => 'all_collection', 'title' => 'all_collection'], ['id' => 'ready_to_wear', 'title' => 'ready_to_wear'],['id' => 'customize', 'title' => 'customize'],['id' => 'shop', 'title' => 'shop']);	

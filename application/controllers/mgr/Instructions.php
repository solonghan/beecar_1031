...
class Instructions extends Base_Controller {
	private $th_title = ["使用說明","修改日期","操作"]; //, "置頂"
	private $th_width = ["", "",""];
	...
	public function __construct(){
		parent::__construct();	
		$this->is_mgr_login();
		$this->data['active'] = "INSTRUCTIONS";
		
	}
	
	public function index(){
		$this->data['title'] = '使用說明';
		$this->data['excel_export'] = false;
		$this->data['export_url'] = "mgr/instructions/export";
		$this->data['action'] = base_url()."mgr/instructions/";
	...
	public function edit($id){
		...
		//從blog抓出對應id的內容
		$oridata = $this->db->where(array("id"=>$id))->get($this->instructions)->row_array();

		$param = [								
								["使用說明", "content", "textarea",$oridata['content']],
		...
		if ($_POST) {
			...
			$res = $this->db->where(array("id"=>$id))->update($this->instructions, $data);

			if ($res) {
			
				$this->js_output_and_redirect("編輯成功", base_url()."mgr/instructions");
			}else{
			...
			$this->data['action'] = base_url()."mgr/instructions/edit/".$oridata['id'];
	...
	public function data(){
		...
		$total = $this->db->from($this->instructions." I")
		...
		foreach ($list as $item) {
			...
			$html .= $this->load->view("mgr/items/instructions_item", array(
				"item"  =>	$item,
				"total" =>	$total_num
			), TRUE);
		}

...
class User extends Base_Controller {
	private $th_title = ["帳號", "會員資訊","身份", "認證狀態", "超級通知狀態", "註冊日期", "審核狀態","動作"]; //, "置頂"
	...
	public function index(){
		...
		$this->data['export_url'] = "mgr/user/export";
	...
	public function edit($id){
		...
		if ($_POST) {
			...
			// $str_user="select * from user  where id = $id  ";
			// $res_user=$this->db->query($str_user)->row_array();
			// print_r($_POST['verify_status']);
			// print_r($param[4][3]);
			// exit;
			

			if($_POST['verify_status']!=$param[4][3]){
				// print 456;
				$str="select * from fcm_token  where user_id = $id  ORDER BY id DESC LIMIT 1 ";
				$res_log=$this->db->query($str)->row_array();
				// print_r($res_log);

				if($res_log){
					$this->db->where(array("id"=>$res_log['id']))
						->update('fcm_token', array(
						"token"      =>	'',
					));
					// print 123;
				}
			...
			// exit;
			// print_r($res_user['verify_status']);exit;
			// print_r($id);exit;
	...
	public function data(){
		...
		$syntax = "P.is_delete = 0 AND P.password !='' ";
		if ($search != "") {
			$syntax .= " AND (";
			$index = 0;
	...
	public function del(){
		...
		if (is_numeric($id)) {
			if($this->db->where(array("id"=>$id))->update($this->user, array("is_delete"=>1))){
				...
				// header("Location:".base_url()."mgr/user");
				// $this->js_output_and_redirect("刪除成功", base_url()."mgr/user");
				// $this->output((TRUE,))

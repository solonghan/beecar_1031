...
{
    private $th_title = ["群組", "建立者","群組成員名單", "動作"]; //, "置頂"
    private $th_width = ["","", "", ""];
    private $order_column = ["id", "","", ""]; //, "is_head"
    ...
    public function index()
    {
        ...
        $this->data['export_url'] = "mgr/group/export";
        $this->data['action'] = base_url() . "mgr/group/";
    ...
    public function edit($id)
    {
        ...
        //從blog抓出對應id的內容
        $oridata = $this->db->where(array("id" => $id))->get($this->groups)->row_array();

        $param = [
            ["群組", "title", "text", $oridata['title']],
            // ["建立者", "username", "text", $oridata['username']],
            // ["", "mobile", "text", $oridata['mobile']],
        ...
        if ($_POST) {
            ...
            $res = $this->db->where(array("id" => $id))->update($this->groups, $data);

            if ($res) {

                $this->js_output_and_redirect("編輯成功", base_url() . "mgr/group");
            } else {
                $this->js_output_and_back("發生錯誤");
            }
        } else {
            //僅讀取edit頁面
            $this->data['title'] = '編輯群組資料 ' . $oridata['id'];
            $this->data['parent'] = '群組列表';
            $this->data['parent_link'] = base_url() . "mgr/group";
            $this->data['action'] = base_url() . "mgr/group/edit/" . $oridata['id'];
    ...
    public function data()
    {
        ...
        $total = $this->db->from($this->groups . " G")
        ...
        if ($order_column[$order] != "") {
            $order_by = "G." . $order_column[$order] . " " . $direction . ", " . $order_by;
        }
        /////
        $str = "select * from groups where is_delete=0 ";
        $res = $this->db->query($str)->result_array();
        // print_r($res);exit;
        foreach ($res as $r) {
            $res = $this->db->select("G.*,GROUP_CONCAT(GD.driver_id ) as friends")
                ->from("groups" . " G")
                ->join("group_driver" . " GD", "G.id = GD.group_id", 'left')
                ->where(array("G.id" => $r['id']))
                // ->group_by("G.title")
                ->order_by($order_by)
            ...
            $str_user = "select * from user where id=$res[user_id] AND is_delete=0 ";
            $res_user = $this->db->query($str_user)->row_array();
        ...
        foreach ($list_data as $item) {
            $html .= $this->load->view("mgr/items/group_item", array(
                "item"  =>    $item,
                "total" =>    $total_num
            ), TRUE);
        }
